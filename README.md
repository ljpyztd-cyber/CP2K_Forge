...
- 支持 La-Lu 镧系元素 LnPP1：检测后自动启用并将 OT 最小化器初始设为可编辑的 CG；只修补 `&DFT` 数据文件引用和镧系 `&KIND` 基组/赝势映射，不改动 SCF 精度、截断能、MAX_SCF 或 DFT+U 设置。
...
- Support La-Lu LnPP1 with automatic detection and an editable initial CG choice for the OT minimizer. Only the `&DFT` data-file references and lanthanide `&KIND` basis/potential mappings are patched; SCF accuracy, cutoffs, MAX_SCF, and DFT+U remain unchanged.

...
电子结构菜单提供 La-Lu 镧系元素 LnPP1 预设，按元素写入 `BASIS_MOLOPT_LnPP1` 基组和 GTH-PBE-q 赝势映射，并应用 OT/CG、600 Ry CUTOFF 与 60 Ry REL_CUTOFF。SCF 精度不由 LnPP1 预设覆盖，默认仍为 Medium；Ce 体系会建议 `f 4.08 eV` DFT+U。运行生成的任务前，请确保 CP2K 能找到 `BASIS_MOLOPT_LnPP1` 和 `POTENTIAL+LnPP1`。

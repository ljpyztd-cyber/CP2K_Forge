...
本目录包含 CP2K_Forge `v0.4.12` 桌面程序源码：PyQt 主界面、结构可视化、设置持久化、桌面专用 INP 生成后端和 PyInstaller 构建配置。可视化模块支持整套结构或仅所选原子在球棍与 VDW 表示之间切换。
